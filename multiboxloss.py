...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        for idx in range(num_batch):
            truths = targets[idx][:, :, :-1].to(self.device) #xmin, ymin, xmax, ymax of BBox
        ...
        #loss_conf
        #Cross_Entropy
        batch_conf = conf_data.view(-1, num_classes) #(num_batch, num_box, num_classes) -> (num_batch*num_box, num_classes)
        loss_conf = F.cross_entropy(batch_conf, conf_t_label.view(-1), reduction = 'none')

        #hard negative mining
        num_pos = pos_mask.long().sum(1, keepdim = True)
        loss_conf = loss_conf.view(num_batch, -1) #(num_batch, 8732)
